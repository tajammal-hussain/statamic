...
@section('title', 'Create Navigations')
        ...
        <div class="max-w-lg mt-4 mx-auto">
            <div class="rounded p-6 lg:px-20 lg:py-10 shadow bg-white">
                <header class="text-center mb-16">
                    ...
                    <p class="text-gray">
                        {{ 'Navigations are multi-level lists of links that can be used to build navbars, footers, sitemaps, and other forms of frontend navigation.' }}
                    </p>
                </header>
                @if ($errors->any())
                ...
                <form action="{{ route('navigations.add') }}" method="POST">
                    ...
                    <div class="mb-10">
                        ...
                        <div class="text-2xs text-gray-600 mt-2 flex items-center">
                            {{ 'We recommend a name that matches where it will be used, like "Main Nav" or "Footer Nav".' }}
                        </div>
                    </div>
                    <div class="mb-4">
                        <label for="handle" class="font-bold text-base mb-1">{{ 'Handle' }}</label>
                        <input type="text" tabindex="2" class="input-text" type="text" id="handle" name="handle"
                            value="{{ old('handle') }}" placeholder="Enter handle" readonly>
                        <div class="text-2xs text-gray-600 mt-2 flex items-center">
                            {{ "Used to reference this navigation on the frontend. It's non-trivial to change later." }}
                        </div>
                    </div>
            </div>
